...
class App extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (_) => TaskBloc(list: [] /*new List<RootTask>()*/)
        ..add(RootTaskLoadSuccessEvent()), //CounterBloc(),
      child: BlocProvider(
        create: (_) => CheckTaskBloc(list: [] /*new List<CheckTask>()*/)
          ..add(CheckTaskLoadSuccessEvent(null)),
        child: BlocProvider(
              ...
              return MaterialApp(
                home: BlocBuilder<ProviderBloc, ProviderState>(
                  ...
                  if (state is RootState) {
                    ...
                    print("Dialog State");
                    return MyDialog(
                        state.changeObj, state.rootTask, state.checkTask);
                  } else if (state is HorizontalState) {
                    return HorizontalPage(state.rootTask);
                  } else {
                    return RootPage();
                  }
